...
def do_seeds():
    ...
    member.positionsadd(models.Position.objects.get(title="Trainer"))

    member.modules.add(models.Module.objects.get(title="U.C.K."))
    member.modules.add(models.Module.objects.get(title="Kickboxen"))

    member.licenses.add(models.Module.objects.get(title="ÜL-C"))

    member.tariff.add(models.Module.objects.get(title="Zero"))
